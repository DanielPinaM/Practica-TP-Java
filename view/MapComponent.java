...
import es.ucm.fdi.Objects.IncomingRoad;
...
public class MapComponent extends JComponent implements TrafficSimulatorObserver {
	...
	private void drawMap(Graphics g) {
		...
		// draw nodes
		for (Junction j : this.mapa.getCruces()) {
		...
		for (Road e : this.mapa.getCarreteras()) {
			...
			for (Vehicle d : e.getVehiculos()) {
				...
				drawCircleOnALine(g, p1.cX, p1.cY, p2.cX, p2.cY, e.getLongitud(), d.getLocation(), diam, dotColor,
						d.getId());
			}
		}
	}
	...
	private boolean hasGreen(Road e) {
		for (IncomingRoad r : e.getCruceDestino().getCarreteras()) {
	...
	private void calculaCoordenadasCruce() {
		...
		int i = 0;
		for (Junction n : this.mapa.getCruces()) {
	...
	@Override
	public void simulatorError(int tiempo, RoadsMap mapa, List<Event> eventos, SimulationError e) {
		this.mapa = mapa;
		calculaCoordenadasCruce();
		refresh();
	}
	...
	@Override
	public void advanced(int tiempo, RoadsMap mapa, List<Event> eventos) {
		this.mapa = mapa;
		calculaCoordenadasCruce();
		refresh();
	}
	...
	@Override
	public void eventAdded(int tiempo, RoadsMap mapa, List<Event> eventos) {
		this.mapa = mapa;
		calculaCoordenadasCruce();
		refresh();
	}
	...
	@Override
	public void reset(int tiempo, RoadsMap mapa, List<Event> eventos) {
		this.mapa = mapa;
		calculaCoordenadasCruce();
		refresh();
	}
